...
class appModel extends Model{
    ...
    public function guardar_usuario($datos){
        $sql="INSERT INTO usuario values (NULL,'".$datos['usuario']."' ,'" . Hash::getHash('sha1', $datos['clave'], HASH_KEY) ."','".$datos['cedula']."' ,'".$datos['nombre']."' ,'".$datos['apellido']."' ,'".$datos['correo']."' ,'".$datos['empresa']."' ,'".$datos['departamento']."' ,'1','".$datos['role']."')";
        $this->_db->query($sql);
    }
    ...
    public function asignar_empresa_usuario($usuario, $role){
        $sql="insert into `empresa_usuario` VALUES (NULL,$role,$usuario)";
        $datos = $this->_db->query($sql);
        return ;
    }
    ...
    public function registrar_menu($datos){
        $sql="insert into menu values(NULL,'".$datos['menu']."','".$datos['enlace']."')";
        $datos = $this->_db->query($sql);
        return ;
    }

    public function registrar_rol($datos){
        $sql="insert into role values(NULL,'".$datos['rol']."','1','')";
        $datos = $this->_db->query($sql);
        return ;
    }
    ...
    public function log($ip,$peticion,$usuario){
        ...
        $sql="INSERT INTO log values (null,$usuario,'$ip','$controlador','$metodo','".date("Y-m-d")."','".date("H:i:s")."')";
        $this->_db->query($sql);          
    }
